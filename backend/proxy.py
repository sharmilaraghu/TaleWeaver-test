...
def _begin_turns(
    ...
    """Build client_content turns for the initial Begin message.

    If opening_text is supplied (pre-generated by Flash Lite via /api/story-opening),
    it is injected as a fake model turn so Gemini Live treats it as words it already
    said and continues naturally from that exact story — keeping the live narration
    consistent with the pre-warm opening image shown on screen.
    """
    ...
    turns: list[dict] = [{"role": "user", "parts": user_parts}]

    if opening_text:
        # Inject as a fake model turn so Gemini continues from these exact words.
        # Gemini will treat this as something it already said and pick up naturally,
        # eliminating the mismatch between the pre-warm image and the live story.
        turns.append({"role": "model", "parts": [{"text": opening_text}]})
...
async def run_proxy_session(
    ...
    try:
        async with websockets.connect(
            ...
            # Kick off the story. We inject opening_text (pre-generated by Flash Lite)
            # as a fake model turn so Gemini continues from those exact words rather than
            # inventing a different story — keeping the live narration consistent with
            # the opening image already shown on screen.
            await gemini_ws.send(json.dumps({
                "client_content": {
                    "turns": _begin_turns(theme, prop_image, opening_text),
                    "turn_complete": True,
                }
            }))
